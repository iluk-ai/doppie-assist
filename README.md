...
2. In Review mode, click an element, describe the requested change in the anchored popover, and add as many numbered annotations as needed. The selected element remains locked and outlined until the note is saved or canceled.
...
4. Choose **Review issues** to see each note with its element screenshot thumbnail. Edit any item inline to refine its feedback type, issue title, request, or override its team, priority, assignee, project, and labels; items use the shared review routing by default.
5. Optionally enable **Group under parent issue** to create one page-review summary with every annotation linked as a child issue.
6. Doppie Assist creates one Linear issue per annotation, or one standalone reproduction issue when the review only contains a recorded flow, and copies the created Linear links to the clipboard. A failed item remains editable and can be retried without interrupting or duplicating successful issues.
...
While Review mode is active, Doppie Assist captures console warnings and errors, unhandled runtime failures, failed resources, and failed `fetch` or XHR responses. Query strings are removed from diagnostic URLs, common credentials are redacted, and form values are never recorded. Diagnostics and reproduction steps are visible under **Captured context** before submission.
...
Linear descriptions are formatted for both people and coding agents. They include the requested change, feedback type, page title, complete URL and URL path, target selector, visible element text, sanitized HTML, element and viewport geometry, reproduction steps, diagnostics, acceptance checks, screenshot evidence, and a stable `doppie-assist/v1` JSON context block. Embedded form values, inline event handlers, scripts, styles, and data URLs are removed from captured HTML.
