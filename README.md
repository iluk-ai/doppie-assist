...
2. In Review mode, click an element, describe the requested change in the anchored popover, and add as many numbered annotations as needed. The selected element remains locked and outlined until the note is saved or canceled. If a tooltip, menu, loading state, or other transient element leaves the DOM while you write, Doppie Assist keeps its last position, screenshot, selector, HTML, and developer context so the annotation can still be saved. Use the parent/child buttons or `Alt` + scroll to move repeatedly up or down through the element hierarchy without dismissing the note.
